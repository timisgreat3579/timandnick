...
buffer(screen,0,0,'LOADING',200).draw(screen)
pygame.display.flip()
...
class launcher():
    ...
    def check_scroll(self):
     if isinstance(self.frame_selected,library_screen):
        if self.library_menu.game_buttons.frames[0].scroll_bar.on_mouse_hover():
            self.library_menu.game_buttons.frames[0].scroll_y = pygame.mouse.get_pos()[1]
...
class quicktype_frame(main_frame):
    def __init__(self,parent):
        ...
        self.quicktype_scores = leaderboard.Leaderboard(user,'quicktype','friends',parent,self.bg_leader.w,self.bg_leader.h
    ...
    def draw(self,surface):
        ...
        self.display_screen.surface.blit(self.led_text.surface,((self.display_screen.w-self.game_text.width)/2,590-(self.scroll_y-163)))
